...
- `1883` - Artemis MQTT port, use `8883` in production with TLS
...
MIT © [Raul Rosenlöcher](https://github.com/rosenloecher-it), the original author of the mqtt-pg-logger.
...
To generate a Fernet key for the application, run this command in your project directory:
...
Example output:
```
TXkgbmFtZSBpcyBUeWxlciBEdXJkZW4uIEknbSBhIHNvZnR3YXJlIGVuZ2luZWVy
```

Copy this output directly into your .env file:
```
FERNET_KEY=TXkgbmFtZSBpcyBUeWxlciBEdXJkZW4uIEknbSBhIHNvZnR3YXJlIGVuZ2luZWVy
```

Notes:
- The key is automatically generated with the correct length and format required by Fernet
- The `.decode()` converts the bytes to a string for easy copying
- Generated keys are base64-encoded and will always be 44 characters
...
flowchart LR
    ...
    class mqtt_client,queue,processor,quart,graphql,auth,event_loop component;
